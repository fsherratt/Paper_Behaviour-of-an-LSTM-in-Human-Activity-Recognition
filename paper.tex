...
\Author{Freddie Sherratt \orcidA{} and Pejman Iravani *\orcidB{}}
% Authors, for metadata in PDF
\AuthorNames{Freddie Sherratt and Pejman Iravani}
...
\abstract{For lower limb prosthesis to adapt to different locomotion modes, the ability to understand the users intent is required. To enable this, systems for recognising different human locomotive modes are needed. A new data set of human locomotion activities is collected, capturing steady state and transitions between activities in natural environments. The data set is then used to develop an understanding of the how a reduced complexity Long Short Term Memory (LSTM) network learns recognise human activities. It is found that the model primarily identified activity type based on input data around early stance. The performance effects of hyper-parameters on a full complexity model are then evaluated. From this, it is identified that variations between individual's gait prevent the model from generalising to new unseen users. Further investigation around these issues is conducted, which finds that gait variations between users primarily occurs in early stance. For models trained to individuals, errors mostly occur around transitions between activities, an additional transition state is added to the model to try and account for this.}
...
% Use of able-bodied subjects as a substitute for amputees
...
\authorcontributions{Freddie Sherratt and Pejman Iravani were responsible for the conceptualization of the research. Freddie Sherratt developed the methodology, software and performed the  data curation, investigation, and formal analysis, writing the original draft. Pejman Iravani supervised the project and reviewed the paper.}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\funding{This research was funded by an EPSRC Studentship from the University of Bath}
