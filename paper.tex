...
\abstract{To adapt lower limb prosthesis for different locomotion modes, they require systems for recognising user intent. As medical devices, these systems require both high accuracy and a detailed understanding of their internal operation. A dataset of six human locomotion activities (walking, stopped, stairs and ramps) from twenty-two subjects is collected, capturing steady-state and transitions between activities in dynamic natural environments. The data set is then used to analyse how a reduced complexity Long Short Term Memory (LSTM) network learns to recognise human activities. The analysis identifies that the model primarily classifies activity type based on input data around early stance. The effects of hyper-parameters on the classification accuracy on a full complexity Human Activity Recognition (HAR) model are evaluated for both seen and unseen subjects. The results reveal that over-fitting to individuals' gait traits prevents generalisation to new unseen users. Further investigation reveals that gait variations between users primarily occur in early stance. For a full complexity model trained to an individual, errors mostly occur around the transition between activities. To compensate for these errors, the addition of a transition state is investigated. No significant performance improvement occurs, suggesting that the transition region is highly variable.}

% Keywords
% \keyword{keyword 1; keyword 2; keyword 3 (list three to ten pertinent keywords specific to the article, yet reasonably common within the subject discipline.)}
\keyword{Human Activity Recognition; IMU; LSTM; Wearables; Prosthetic}
...
% Highly performing LSTM networks in HAR - Controlled conditions
...
% Not clearly specified numbers of units but appears to be a single unit network
...
% Add pictures of different activities
...
\begin{figure}[!htb]
     ...
     \end{subfigure}
    \caption{Gait trends for $x$ accelerometer and $y$ gyroscope for 3 different activities. The solid lines show the mean and the shaded area the standard deviation for n steps. Black solid - Subject 4, Red dashed - Subject 7, Blue dot dash - Subject 16}
...
% What were we were trying to investigate?
...
\authorcontributions{Freddie Sherratt and Pejman Iravani were responsible for the conceptualization of the research. Freddie Sherratt developed the methodology, software and performed the  data curation, investigation, and formal analysis, writing the original draft. Andrew Plummer and Pejman Iravani  supervised the project and reviewed the paper.}
